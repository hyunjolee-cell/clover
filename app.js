...
(() => {
  ...
  function seedState() {
    ...
    s.flows = [
      ...
      flow('적금통장 이체', salary.id, bank.id, 27, 850000,
           '경조사 10 · 여행 10 · 부모님 40 · 조카 3 · 비상금 12 · 옷 10 (만원)'),
  ...
  const app = {
    ...
    addFlow: null,         // 첫 화면 추가 메뉴 { cat, kind } (null=닫힘)
  ...
  const HELP_TOPICS = [
    { cat: '항목 설정', icon: 'wallet', q: '공과금 항목을 추가하려면?',
      kw: '공과금 추가 전기 수도 도시가스 정수기 관리비',
      steps: [
        '<b>더보기 → 항목 설정</b>으로 들어갑니다.',
        '위쪽 칩에서 <b>공과금 항목</b>을 누릅니다.',
        '오른쪽 <b>추가</b> 목록을 열고 원하는 항목을 고르거나 <b>직접 입력하기…</b>를 선택합니다.',
        '이름과 예상 금액을 적고 <b>저장</b>하면 끝입니다.'
      ], go: { label: '공과금 항목 바로 추가', tab: 'settings', group: 'utility', add: 'utility' } },
    { cat: '항목 설정', icon: 'inbox', q: '고정비를 추가하려면?',
      kw: '고정비 추가 월세 보험 구독료 정수기',
      steps: [
        '<b>더보기 → 항목 설정</b>으로 들어갑니다.',
        '위쪽 칩에서 <b>월 고정비</b>를 누릅니다.',
        '<b>추가</b> 목록에서 고르거나 <b>직접 입력하기…</b>로 이름을 적습니다.',
        '월 금액을 적고 <b>저장</b>합니다.'
      ], go: { label: '고정비 바로 추가', tab: 'settings', group: 'fixed', add: 'fixed' } },
    { cat: '항목 설정', icon: 'book', q: '고정비와 공과금, 어디에 넣어야 하나요?',
      kw: '고정비 공과금 차이 분류 정수기 어디 구분',
      ...
      go: { label: '항목 설정 열기', tab: 'settings', group: 'fixed' } },
    { cat: '항목 설정', icon: 'history', q: '한 번 넣으면 계속 반영되나요?',
      kw: '자동 반영 매달 계속 반복 유지',
    ...
    { cat: '수입 넣기', icon: 'chart', q: '월급(정기소득)을 설정하려면?',
      kw: '월급 수입 정기소득 급여 추가',
      steps: [
      ...
      ], go: { label: '정기소득 바로 추가', tab: 'settings', group: 'income', add: 'income' } },
    { cat: '지출 넣기', icon: 'edit', q: '생활비(공동생활비)를 기록하려면?',
      kw: '생활비 지출 기록 입력 공동생활비',
      steps: [
        '아래 <b>입력</b> 탭으로 갑니다. (홈·달력의 <b>＋ 지출 기록</b>도 같습니다.)',
        '<b>공동생활비</b> 칩을 누르면 새 내역이 만들어집니다.',
        '금액과 사용처를 적으면 바로 반영됩니다.'
      ], go: { label: '생활비 입력 화면 열기', tab: 'monthly' } },
    { cat: '지출 넣기', icon: 'history', q: '매달 나가는 지출을 자동으로 넣으려면?',
      kw: '반복 지출 자동 구독 용돈 매달',
      steps: [
      ...
      ], go: { label: '반복 지출 화면 열기', tab: 'monthly' } },
    { cat: '흐름 이해', icon: 'flow', q: '화면은 7월인데 왜 25일부터 금액이 쌓이나요?',
      kw: '급여일 사이클 7월 8월 25일 왜 제목',
      ...
      go: { label: '달력에서 사이클 보기', tab: 'calendar' } },
    { cat: '흐름 이해', icon: 'piggy', q: '안 쓴 생활비가 다음 달로 넘어가나요?',
      kw: '이월 남은 생활비 다음달 초과 차감',
      body: '<b>공동생활비만</b> 이월됩니다. 이번 달에 남으면 다음 달 예산에 더해지고(이월 +), 초과해서 쓰면 다음 달에서 빠집니다(이월 −).<br><br>' +
            '현조·신영 <b>개인 용돈</b>은 이월하지 않습니다. 매달 정한 금액만 관리합니다.' },
    { cat: '적금·자산', icon: 'piggy', q: '적금이 25일에 반영되나요?',
      kw: '적금 적립일 25일 납입 반영 오름',
      ...
      go: { label: '적금 설정 열기', tab: 'settings', group: 'saving' } },
    { cat: '적금·자산', icon: 'piggy', q: '적금에서 돈을 꺼내 썼을 때는?',
      kw: '적금 꺼내 인출 사용 뺀',
      steps: [
    ...
    { cat: '적금·자산', icon: 'wallet', q: '집·차 같은 목돈 지출은 어떻게 기록하나요?',
      kw: '목돈 지출 집 차 큰 자산 대출',
      steps: [
        '아래 <b>자산</b> 탭에서 <b>목돈 지출 → 지출 기록</b>을 누릅니다.',
        '나간 돈(출금 자산)과 그 돈으로 생긴 자산·대출을 함께 적습니다.',
        '현금이 자산으로 바뀐 것이면 순자산은 그대로 유지됩니다.'
      ], go: { label: '자산 화면 열기', tab: 'assets' } },
    { cat: '설정', icon: 'settings', q: '급여일을 바꾸려면?',
      kw: '급여일 변경 사이클 25일 며칠',
      steps: [
    ...
    { cat: '함께 쓰기', icon: 'key', q: '배우자와 함께 쓰려면?',
      ...
      ], go: { label: '공유공간 열기', tab: 'settings', group: 'income' } },
    { cat: '리포트', icon: 'chart', q: '이번 달 리포트를 PDF·이미지로 내보내려면?',
      kw: '리포트 pdf 이미지 내보내기 저장 공유',
      steps: [
        '<b>더보기 → 월별 리포트</b>로 갑니다.',
        '함께 담을 상세 내역이 있으면 아래에서 체크합니다.',
        '<b>PDF로 저장</b> 또는 <b>이미지로 저장</b>을 누르면 파일로 내려받습니다.'
      ], go: { label: '월별 리포트 열기', tab: 'report' } }
  ];
  ...
  function calendarView() {
    ...
    for (let dt = new Date(start); dt <= end; dt.setDate(dt.getDate() + 1)) {
      ...
      // 사이클 뒷부분(다음 달력월)은 옅게 구분하고, 월이 바뀌는 날엔 "N월" 배지를 붙인다
      const secondHalf = dt.getMonth() !== start.getMonth();
      const showMon = idx === 0 || dnum === 1;
      idx++;
      cells.push(`
        <button type="button" class="cal-cell${selected === date ? ' on' : ''}${isToday ? ' today' : ''}${secondHalf ? ' second-half' : ''}"
                data-pick-date="${date}">
          <span class="cal-daywrap">
            ${showMon ? `<span class="cal-mon">${dt.getMonth() + 1}월</span>` : ''}
            <span class="cal-day${dow === 0 ? ' sun' : dow === 6 ? ' sat' : ''}">${dnum}</span>
          </span>
  ...
  /* 도움말 목록 HTML만 따로 만든다. 검색 중에는 이 부분만 갈아 끼워
     입력칸을 다시 그리지 않는다(한글 조합이 끊기지 않도록). */
  function helpListHtml() {
    ...
    if (!matched.length)
      return '<p class="help-none">찾는 내용이 없습니다. 다른 말로 검색해 보세요.</p>';
    ...
    const answer = (t, i) => `
      ${t.steps
        ? `<ol class="help-steps">${t.steps.map(s => `<li><span class="help-step-body">${s}</span></li>`).join('')}</ol>`
        : `<p class="help-body">${t.body}</p>`}
      ${t.go ? `<button class="help-go" type="button" data-help-go="${i}">
          ${t.go.label} ${icon('chevron', 16)}</button>` : ''}`;
      ...
      ${g.items.map(({ t, i }) => `
        <div class="help-item ${app.helpSel === i ? 'open' : ''}">
          <button class="help-q" type="button" data-help-topic="${i}">
            <span class="help-q-ico">${icon(t.icon || 'book', 18)}</span>
            <span class="help-q-txt">${esc(t.q)}</span>
            <span class="help-q-arrow">${icon('chevron', 16)}</span>
          </button>
          ${app.helpSel === i ? `<div class="help-a">${answer(t, i)}</div>` : ''}
        </div>`).join('')}
    `).join('');
  }

  /* --- 첫 화면 추가 메뉴(대분류 → 중분류 → 소분류) --------------------------
     ＋ 버튼을 누르면 어떤 항목이든 앞단에서 바로 추가한다. */
  const ADD_TREE = [
    { cat: '수입', desc: '월급·보너스 등 들어오는 돈', icon: 'chart', kinds: [
      { kind: 'income', label: '정기소득 (월급)' },
      { kind: 'bonus', label: '보너스·상여금' } ] },
    { cat: '지출', desc: '생활비·고정비·공과금 등 나가는 돈', icon: 'edit', kinds: [
      { kind: 'transaction', label: '생활비 쓴 내역' },
      { kind: 'fixed', label: '월 고정비' },
      { kind: 'utility', label: '공과금' },
      { kind: 'recurring', label: '반복 지출 (구독 등)' } ] },
    { cat: '저축·자산', desc: '적금·자산·목돈 지출', icon: 'piggy', kinds: [
      { kind: 'saving', label: '적금·저축' },
      { kind: 'asset', label: '자산·부채' },
      { kind: 'bigSpend', label: '목돈 지출 (집·차)' } ] }
  ];
  // 소분류(프리셋). transaction 은 생활비 카테고리로 고른다. 없는 종류는 바로 새 항목.
  const addSubs = kind => kind === 'transaction'
    ? [DEFAULT_CATEGORY, ...QUICK_CATEGORIES]
    : (PRESETS[kind] || null);
  const ENTITY_KIND_LABEL = {
    income: '정기소득', bonus: '보너스', transaction: '생활비 내역', fixed: '월 고정비',
    utility: '공과금', recurring: '반복 지출', saving: '적금·저축', asset: '자산·부채', bigSpend: '목돈 지출'
  };

  /* 이번 사이클에 생활비 한 줄을 만들고 편집 폼을 연다(＋ 메뉴·유형칩 공통). */
  async function quickAddTx(category) {
    const id = uid();
    const date = app.month === currentMonth ? ymd(today()) : ymd(cycleRange(app.month).start);
    app.openRow = `transaction:${id}`; app.draft = { id, kind: 'transaction' };
    await mutate(
      state => { state.transactions.push({ id, date, owner: '공동', category, place: '', amount: 0, memo: '' }); },
  ...
  /* 추가 메뉴에서 종류·프리셋을 고르면 알맞은 화면으로 가서 새 항목 폼을 연다. */
  function addFlowSelect(kind, preset) {
    app.addFlow = null;
    const group = { income: 'income', fixed: 'fixed', utility: 'utility', saving: 'saving' };
    const tabOf = { income: 'settings', fixed: 'settings', utility: 'settings', saving: 'settings',
      asset: 'assets', bigSpend: 'assets', bonus: 'monthly', recurring: 'monthly', transaction: 'monthly' };
    app.tab = tabOf[kind] || 'monthly';
    if (group[kind]) app.settingsGroup = group[kind];
    render();
    if (kind === 'transaction') quickAddTx(preset || DEFAULT_CATEGORY).catch(() => {});
    else addEntity(kind, preset || undefined).catch(() => {});
  }

  function addFlowPanel() {
    if (!app.addFlow) return '';
    const { cat, kind } = app.addFlow;
    let title = '무엇을 추가할까요?', back = false, body;

    if (!cat) {
      body = `<div class="add-cats">${ADD_TREE.map(c => `
        <button class="add-cat" type="button" data-add-cat="${esc(c.cat)}">
          <span class="add-cat-ico">${icon(c.icon, 22)}</span>
          <span class="add-cat-txt"><b>${esc(c.cat)}</b><small>${esc(c.desc)}</small></span>
          ${icon('chevron', 18)}
        </button>`).join('')}</div>`;
    } else if (!kind) {
      const group = ADD_TREE.find(c => c.cat === cat);
      title = `${cat} · 무엇을 추가할까요?`; back = true;
      body = `<div class="add-kinds">${(group ? group.kinds : []).map(k => `
        <button class="add-kind" type="button" data-add-kind="${k.kind}">
          <span>${esc(k.label)}</span>${icon('chevron', 16)}
        </button>`).join('')}</div>`;
    } else {
      const subs = addSubs(kind);
      title = `${ENTITY_KIND_LABEL[kind] || ''} · 어떤 걸 추가할까요?`; back = true;
      body = `
        <div class="add-subs">
          ${(subs || []).map(s => s.startsWith('—')
            ? `<div class="add-sub-sep">${esc(s.replace(/—/g, '').trim())}</div>`
            : `<button class="add-sub" type="button" data-add-pick="${esc(s)}">${esc(s)}</button>`).join('')}
          <button class="add-sub custom" type="button" data-add-custom>
            ${icon('plus', 15)} 직접 입력하기
          </button>
        </div>`;
    }

    return `
      <div class="help-overlay" data-add-backdrop>
        <div class="help-sheet add-sheet" role="dialog" aria-label="항목 추가">
          <div class="help-head">
            <div style="display:flex;align-items:center;gap:8px">
              ${back ? `<button class="add-back" type="button" data-add-back aria-label="뒤로">${icon('back', 20)}</button>` : ''}
              <div><b>${esc(title)}</b></div>
            </div>
            <button class="help-x" type="button" data-add-close aria-label="닫기">${icon('close', 20)}</button>
          </div>
          <div class="help-list">${body}</div>
        </div>
      </div>`;
  }

  /* 도움말 "바로 가기" 실행 — 안내한 화면으로 이동. add 가 있으면 새 항목까지 만들어 폼을 연다. */
  function applyHelpGo(i) {
    const go = HELP_TOPICS[i] && HELP_TOPICS[i].go;
    if (!go) return;
    app.helpOpen = false;
    app.tab = go.tab;
    ...
    return `
      <div class="help-overlay" data-help-backdrop>
        <div class="help-sheet" role="dialog" aria-label="도움말">
          ...
          <div class="help-list">${helpListHtml()}</div>
        </div>
      </div>`;
  }
    ...
    return `
      <div class="app ${hasAddFab ? 'has-add-fab' : 'no-add-fab'}">
        ...
        ${['home', 'calendar', 'monthly'].includes(app.tab)
          ? `<button class="fab" type="button" data-add-menu aria-label="항목 추가">${icon("plus", 24)}</button>`
          : ''}
  ...
  document.addEventListener('click', async e => {
    ...
    if (t.closest('[data-help-close]') ||
        (t.hasAttribute && t.hasAttribute('data-help-backdrop'))) {
      app.helpOpen = false; render(); return;
    }
    // 도움말 "바로 가기" — 안내한 화면으로 이동하고, 추가형이면 새 항목 폼까지 연다
    const helpGo = t.closest('[data-help-go]');
    if (helpGo) { applyHelpGo(num(helpGo.dataset.helpGo)); return; }
    ...
    if (quickCat) { await quickAddTx(quickCat.dataset.quickCat); return; }

    // 첫 화면 추가 메뉴 (대분류 → 중분류 → 소분류)
    if (t.closest('[data-add-menu]')) { app.addFlow = { cat: null, kind: null }; render(); return; }
    if (t.closest('[data-add-close]') ||
        (t.hasAttribute && t.hasAttribute('data-add-backdrop'))) {
      app.addFlow = null; render(); return;
    }
    if (t.closest('[data-add-back]')) {
      if (app.addFlow && app.addFlow.kind) app.addFlow.kind = null;
      else if (app.addFlow && app.addFlow.cat) app.addFlow.cat = null;
      else app.addFlow = null;
      render(); return;
    }
    const addCat = t.closest('[data-add-cat]');
    if (addCat) { app.addFlow = { cat: addCat.dataset.addCat, kind: null }; render(); return; }
    const addKind = t.closest('[data-add-kind]');
    if (addKind) {
      const k = addKind.dataset.addKind;
      if (addSubs(k)) { app.addFlow = { cat: app.addFlow.cat, kind: k }; render(); }
      else addFlowSelect(k);       // 소분류 없는 종류는 바로 새 항목
      return;
    }
    const addPick = t.closest('[data-add-pick]');
    if (addPick) { addFlowSelect(app.addFlow.kind, addPick.dataset.addPick); return; }
    if (t.closest('[data-add-custom]')) { addFlowSelect(app.addFlow.kind); return; }
